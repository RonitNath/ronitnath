    ...
    harness::run(async {
        ...
        for named in ALL {
            ...
            let response = server
                ...
                .await;
            // `document` is the only one that takes a row, and asking for none
            // is asking about nothing.
            let expected = if query == "document" {
...
fn a_key_that_left_a_shared_set_is_reported_as_having_left_it() {
    use rn_site::api::query::{Row, diff, remember};
