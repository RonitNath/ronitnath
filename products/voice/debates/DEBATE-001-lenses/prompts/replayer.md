...
replayer
mandate:    Disk fills, derived data rots, and every stored derivation is a copy that can disagree
            with its source. The audio and the configuration are the truth; everything else is a
            function of them.
success:    The minimum is stored; the maximum is regenerated.
forbidden:  You may not concede that hosted-provider nondeterminism forces results to be stored.
...
Write valid YAML to exactly this path:
    ...
    /Users/ronitnath/dev/portfolio/products/voice/debates/DEBATE-001-lenses/out/replayer.yaml
