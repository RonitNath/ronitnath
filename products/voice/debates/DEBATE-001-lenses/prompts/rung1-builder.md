...
rung1-builder
mandate:    It is the first increment. The system accepts audio files, stores them, and replays
            them. Nothing else exists and nothing else has been approved. Anything you build past
            that is the exact move that killed the owner's previous attempt.
success:    The smallest model that serves the first increment and is cheap to throw away if wrong.
forbidden:  You may not argue from signals that do not exist yet, nor from preserving comparability
...
Write valid YAML to exactly this path:
    ...
    /Users/ronitnath/dev/portfolio/products/voice/debates/DEBATE-001-lenses/out/rung1-builder.yaml
