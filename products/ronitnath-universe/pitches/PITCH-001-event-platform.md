...
- **delta-8** (2026-08-06, owner at the data gate, session ③; EV-027…030, DEC-013/DEC-014, GATE-002): **the domain model and seams are signed off.** Two rulings reach into §1. **The data seam is hiqlite, not PostgreSQL** (DEC-013) — §1's "PostgreSQL seam" and PKT-01's constraint are superseded for this product, `monorepo.md` is untouched elsewhere, and DEC-006's `LISTEN`/`NOTIFY` fan-out becomes hiqlite's Raft-replicated notify bus. The contradiction had gone unnoticed since freeze: the override was issued the same day, when the store held one disposable table. **Fetched is not opened** (DEC-014) — iMessage and Instagram fetch invite URLs to build previews, so DEC-009's per-GET counter marked links opened before a human saw them; the invitee list and directory now show a client-confirmed count. Also: archiving a person revokes their links (EV-028), anonymous visitors hold SSE streams, and geolocation is a provisioned `.mmdb` file rather than a service (EV-030). No scope change — the same product, modelled correctly.
