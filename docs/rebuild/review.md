...
| **forged `p_` id** | Uniform decline, without a query | `ids::decode` checks the prefix, then the AES tag, then the row bound — three refusals with one answer (`ids/mod.rs:52-72`). `refs::` turns every one of them into `decline()`. Proptests: `properties.rs:37 a_forged_id_is_refused_for_every_kind` over 22 random base64url characters, and `:61 an_id_never_crosses_deployments`. |
...
|---|---|---|
| `unsafe` | **12**, 0 unjustified | 4 in `starscape` (`globe.rs:110,120,234`, `render.rs:91`) — `js_sys::Float32Array::view` / `Uint16Array::view` over a Rust slice handed straight to `buffer_data_with_array_buffer_view`, the standard wasm-bindgen zero-copy upload. Sound as long as nothing allocates while the view lives, and in all four the view is constructed and consumed in one expression. 8 in `#[cfg(test)]` blocks (`config.rs`, `tests/cluster_env.rs`) — `std::env::set_var`, which is `unsafe` in edition 2024. |
...
52 screenshots, named `<page>-<width>-<theme>.png`. The ten that back the
findings below are committed at **`docs/rebuild/review-shots/`**; the full set
was taken in this session's scratchpad
(`…/1a80dd12-969c-454a-80ea-f8f4533b4fa6/scratchpad/r1shots/`), which is
session-scoped and will not survive. The routes: `/`, `/auth`, `/links/<token>` and three
...
|---|---|---|---|
| V1 | `/` vs `/auth` vs `/links/<t>` | The wordmark is **three different colours**. Red on the dark landing, **amber on the light landing**, white on `/auth`, near-black on the light claim page. Worse, light-mode landing *inverts* the pair: the name is amber and "Isoastra" is red, where dark mode has the name red and "Isoastra" amber. The 2026-06-06 ruling is one colour for a wordmark. | `review-shots/landing-1440-dark.png`, `review-shots/landing-390-light.png`, `review-shots/auth-1440-dark.png`, `review-shots/claimanon-1440-light.png` |
| V2 | `/links/<token>` | The claim page cannot name what it grants. An actual invitation renders as `Grant::Unknown` → the words "An invitation", because `Grant::of` reads only `link.verifies_factor_id` and an invitation carries its grant as a relation row (`links/mod.rs:120`). It does not say which organization or group, who minted it, or at what role — all of which are one join away. The one grant this cut actually mints is the one the page cannot describe. | `review-shots/claimanon-1440-light.png` |
| V3 | `/platform/audit` | The **Acting as** column renders a raw public id (`p_DZhS735xBTdLxwnEXyM9jA`) while the **Actor** column beside it renders a display name. An operator console showing an id where a name is available, in the same row, is the interface failing to state. | `review-shots/platform-audit-1440-light.png` |
| V4 | every tier page | A native `<select>` for **Acting as** in the rail, and another for **Role** in the invite form, beside entirely custom controls everywhere else. "Never ship a native control beside your own control" (2026-02-27). | `review-shots/app-home-1440-light.png`, `review-shots/app-groups-1440-dark.png` |
| V5 | `/org`, `/platform` drill-ins, `/links/<t>` at 1440 | Content occupies the top ~300 px and the left ~40 % of a 1440×900 frame and the rest is empty. `/org` is five label/value pairs, three counts and two links in a 1 200 px-wide column. "Empty space is a layout bug, not breathing room" — either the blocks go newspaper-order across the width, or the frame stops being that wide. | `review-shots/org-home-1440-dark.png`, `review-shots/claimanon-1440-light.png` |
| V6 | `/` light mode | A white band across the top ~70 px where the page background shows above the sky canvas, with the theme toggle floating on it. Invisible in dark because the band and the sky are both black. | `review-shots/landing-390-light.png` |
| V7 | tokens | `--radius: 0.25rem` against "no rounded corners" (2026-04-03). Visible on inputs, buttons, the focus ring and the rail's active row. Small enough to be deliberate, so it is presented rather than changed. | `review-shots/auth-1440-dark.png` |
| V8 | `/app/documents` | Two headings reading `Documents` on one page — the h1 and the table's h2. | `review-shots/app-documents-390-dark.png` |
