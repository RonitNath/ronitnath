...
| 8 | Cutover (operator, by hand) | phase-8-cutover.md | done — live 2026-07-13 |
...
| 8 | cutover | orchestrator (by hand) | — | aec7c3a deployed on nexus | LIVE 2026-07-13. nexus `~/dev/ronitnath` compose: image built @ baad379+aec7c3a (PUBLIC_URL), site migrated empty v33 DB, real prod .backup imported (write-frozen old stack; post-freeze dump hash == imported snapshot, zero drift), verify-import 16/16 PASS on prod. Ingress: hand-managed T2 apex block removed from nanode main Caddyfile (backup /etc/caddy/Caddyfile.bak-20260713-ronitnath-t1), `ingress-ctl register ronitnath.com 10.0.0.1:3130` + `delete events.ronitnath.com` (wildcard 308 now covers invite URLs). Live edge verify: apex 200, /calendar 200, real token /e/ 200, events.ronitnath.com/e/{token} 308→apex→200. ronitnath-legacy archived; hive ingress reference updated (ba5c4e1); context/ronitnath/events.md rewritten. Pending manual: stop legacy container on gateway (ssh denied from nexus; unrouted, still answering on NetBird :18080) + retire hive ronitnath.nix. |
