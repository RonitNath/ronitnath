...
- **Phase 4 migration mappings (2026-07-12):** semantic `0027_person_identity_links` is repository migration `0028_person_identity_links`; semantic `0028_people_recovery_email` is repository migration `0029_people_recovery_email`.
- **Phase 4 guest login identifier (2026-07-12):** guest login takes recovery email + password. Recovery email is only a locator through `people -> person_identity_links`; the password factor keeps synthetic `external_id='guest:{person_id}'`. Email lookup ambiguity fails closed and still burns the dummy Argon2 verify. Email remains optional at claim time, so a guest who omits it keeps the current session/link flow but cannot password-login after losing that session; this preserves the contract's optional-email and synthetic-factor requirements without making a revoked capability token an enduring login identifier.
