...
async fn state(config: &Config, migrate: bool) -> AppState {
    ...
    AppState::new(store, auth_config(config))
        ...
        .with_admin_url(config.admin_url.clone())
...
mod tests {
    ...
    async fn home_page_renders() {
        ...
        assert!(body.contains(&format!("site.js?v={}", crate::view::release_revision())));
    }
    ...
    async fn oidc_app(auto_provision: bool) -> (axum::Router, crate::store::Store, MockOp) {
        ...
        let state = crate::state::AppState::new(store.clone(), super::test_auth_config(&config))
            .with_oidc(registry)
            .with_admin_url("http://admin.test:3131".into());
    ...
    #[tokio::test]
    async fn admin_oidc_uses_its_configured_callback_not_request_headers() {
        let (app, _store, _op) = oidc_app(true).await;
        let request = axum::http::Request::builder()
        ...
        let location = response
            ...
            .to_str()
            .unwrap();
        let redirect_uri = openidconnect::url::Url::parse(location)
            .unwrap()
            .query_pairs()
        ...
        assert_eq!(
            redirect_uri,
            "http://admin.test:3131/auth/oidc/mock/callback"
        );
    }
