/* An organization's handle is its address: `/org/<handle>`. It is chosen once
 * and it shares the URL space with the site's own top-level paths, so the
 * reserved list is part of the validation rather than a thing to remember. */

export const ORG_HANDLE_MIN = 2;
export const ORG_HANDLE_MAX = 32;

/* Every first path segment this deployment already answers on. */
