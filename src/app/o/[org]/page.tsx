...
export default async function OrgPage({ params }: { params: Promise<{ org: string }> }) {
  ...
  const { organization, principal } = await requireOrgOperator(org);
  const me = encodeId('person', principal.personId);
  ...
  return (
    <main className="indoors">
      ...
      <p className="note">
        <span className="mono">{orgPath(organization.handle)}</span>
      </p>
      ...
      <section>
        ...
        {groups.length === 0 ? (
          <p className="empty">
            None. A group is made on <Link href={userPath(me, 'groups')}>your groups page</Link> and
            owned by
            whoever made it.
          </p>
        ) : null}
          ...
          <table className="rows wide">
            ...
            <tbody>
              {documents.map((row) => (
                <tr key={row.id}>
                  <td>
                    <Link href={userPath(me, `documents/${encodeId('document', row.id)}`)}>
                      {row.title}
                    </Link>
                  </td>
                  <td>
