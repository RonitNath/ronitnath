...
export async function GET(request: Request, { params }: { params: Promise<{ memo: string; file: string[] }> }) {
  ...
  if (name === 'master.m3u8' && memo.hlsMasterKey && memo.generation > 0) {
    const live=(await database().select({sequence:schema.voiceMemoSegment.sequence}).from(schema.voiceMemoSegment).where(and(eq(schema.voiceMemoSegment.memoId,memo.id),eq(schema.voiceMemoSegment.generation,memo.generation))).limit(1))[0];
  ...
  if (playlist && memo.hlsMasterKey && memo.generation > 0) {
    ...
    const segments = await database().select().from(schema.voiceMemoSegment).where(and(eq(schema.voiceMemoSegment.memoId,memo.id),eq(schema.voiceMemoSegment.generation,memo.generation),eq(schema.voiceMemoSegment.rendition,rendition))).orderBy(asc(schema.voiceMemoSegment.sequence));
    if (segments.length) {
      ...
      if (memo.state==='ready') lines.push('#EXT-X-ENDLIST');
